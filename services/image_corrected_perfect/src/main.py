...
def main(run_config: RunConfig):
    ...
    a_term_file = os.path.abspath('corrected_perfect_a_corr.parset')
    write_diagonal_a_term_correction_file(
        a_term_file=a_term_file,
        diagonal_gain_fits_files=[run_config.ionosphere_fits, run_config.beam_fits]
    )

    num_cpus = os.cpu_count()

    image_name = 'corrected_perfect'
