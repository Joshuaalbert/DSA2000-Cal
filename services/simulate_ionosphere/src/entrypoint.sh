...
RUN_DIR=/dsa/run/sim_dsa2000W_"$SPECIFICATION"_"$DURATION"_"$RESOLUTION"_"$SUFFIX"
mkdir -p $RUN_DIR

ln -sf $RUN_DIR /dsa/output                                                                  # for output simulation
OUTPUT_NAME=/dsa/output/sim_dsa2000W_"$SPECIFICATION"_"$DURATION"_"$RESOLUTION"_"$SUFFIX".h5 # output h5parm
