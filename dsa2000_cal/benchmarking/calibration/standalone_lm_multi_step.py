...
def main(num_dir: int, num_ant: int, full_stokes: bool, verbose: bool):
    ...
    solver = MultiStepLevenbergMarquardt(
        residual_fn=partial(residual_fn, vis_model=vis_model),
        verbose=verbose,
        min_cg_maxiter=1,
        init_cg_maxiter=1,
        num_iterations=1,
    ...
    t0 = time.time()
    solve_compiled = jax.jit(solve).lower(state, vis_model).compile()
    t1 = time.time()
    print(f"Compilation time: {t1 - t0}")

    t0 = time.time()
    state, diagnostics = jax.block_until_ready(solve_compiled(state, vis_model))
    t1 = time.time()
    print(f"Execution time: {t1 - t0}")


if __name__ == '__main__':
    main(3, 2048, True, False)
