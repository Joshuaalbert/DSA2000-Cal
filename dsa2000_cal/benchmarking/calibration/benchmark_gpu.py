...
def prepare_data_cal(D: int, T, C, Ts, Tm, Cs, Cm) -> Dict[str, Any]:
    num_antennas = 2048
...
def run_cal(T, C, Ts, Tm, Cs, Cm, backend, m, task, scheme):
    ...
    d_array = []
    for D in range(1, 9):
        data = prepare_data_cal(D, T, C, Ts, Tm, Cs, Cm)
...
def prepare_data(D: int, T, C, Ts, Tm, Cs, Cm) -> Dict[str, Any]:
    ...
    dsa_logger.info(f"Model size {vis_data.nbytes / 2 ** 20} MB, gain size {gains.nbytes / 2 ** 20} MB")
...
def run(build_sharded_entry_point_fn, T, C, Ts, Tm, Cs, Cm, backend, m, task, scheme):
    devices = jax.devices(backend)
...
def main():
    for task, build_sharded_entry_point_fn in zip(
        ...
        for scheme, (T, C, Ts, Tm, Cs, Cm) in zip(
            ...
            for backend in ['cpu', 'cuda']:
                run(build_sharded_entry_point_fn, T=T, C=C, Ts=Ts, Tm=Tm, Cs=Cs, Cm=Cm, backend=backend, m=10,
