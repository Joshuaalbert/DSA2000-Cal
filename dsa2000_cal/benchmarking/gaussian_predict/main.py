...
def main(num_directions: int, num_ant: int, num_times: int, num_channels: int, num_model_freqs: int, order_approx: int):
    ...
    major = np.random.uniform(4, 6., num_directions) * au.arcsec
    minor = np.random.uniform(3.3, 4., num_directions) * au.arcsec
    posang = np.random.uniform(0, np.pi, num_directions) * au.rad
    gaussian_source_model = build_gaussian_source_model(
    ...
    def predict(visibility_coords,
                ...
                geodesic_model):
        return gaussian_source_model.predict(
