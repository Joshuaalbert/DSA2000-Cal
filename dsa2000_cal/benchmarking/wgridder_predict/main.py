...
from dsa2000_common.visibility_model.source_models import build_fits_source_model
...
def main(num_directions: int, num_ant: int, num_times: int, num_channels: int, num_model_freqs: int, num_pix: int):
    model_freqs = np.linspace(0.7, 2, num_model_freqs) * au.GHz
    ra = np.random.uniform(0, 2 * np.pi, (num_directions, num_model_freqs)) * au.rad
    dec = np.random.uniform(-np.pi / 2, np.pi / 2, (num_directions, num_model_freqs)) * au.rad
    dl = np.full((num_directions, num_model_freqs), 1) * au.arcsec
    dm = np.full((num_directions, num_model_freqs), 1) * au.arcsec
    image = np.ones((num_directions, num_model_freqs, num_pix, num_pix, 2, 2), dtype=np.float32) * au.Jy
    fits_source_model = build_fits_source_model(
