...
from dsa2000_cal.visibility_model.source_models.celestial.gaussian_source.gaussian_source_model import transform_ellipsoidal_params_to_plane_of_sky, \
    GaussianSourceModel
...
class SyntheticSkyModelProducer:
    ...
    def create_faint_sources_inside_fov(self, key) -> List[WSCleanSourceModel]:
        ...
        for source in faint_sources:
            ...
            l0, m0, major_tangent, minor_tangent, theta_tangent = transform_ellipsoidal_params_to_plane_of_sky(
                major=major[None], minor=minor[None], theta=theta[None], source_directions=source[None],
                phase_tracking=self.phase_tracking, lmn_transform_params=True)
            A = np.ones((1, len(self.freqs))) * au.Jy
            source_models.append(
                WSCleanSourceModel(
    ...
    def create_a_team_sources(self) -> List[FITSSourceModel]:
        source_models = []
        for source in self.a_team_sources:
    ...
    def create_sky_model(self,
        ...
        fits_models: List[FITSSourceModel] = []
        ...
        return SkyModel(
            component_source_models=component_models,
            fits_source_models=fits_models
        )
