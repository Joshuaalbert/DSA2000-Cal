...
def test_beam_gain_model_factory(array_name: str):
    ...
    assert not np.any(np.isnan(beam_gain_model.model_gains_jax))

    # Plot screens
    beam_gain_model.plot_regridded_beam(p_idx=0, q_idx=0)
    beam_gain_model.plot_regridded_beam(p_idx=1, q_idx=0)
    beam_gain_model.plot_regridded_beam(p_idx=0, q_idx=1)
    beam_gain_model.plot_regridded_beam(p_idx=1, q_idx=1)

    beam_gain_model.plot_beam(p_idx=0, q_idx=0)
    beam_gain_model.plot_beam(p_idx=1, q_idx=0)
    beam_gain_model.plot_beam(p_idx=0, q_idx=1)
    beam_gain_model.plot_beam(p_idx=1, q_idx=1)

    phi, theta = phi_theta_from_lmn(
        beam_gain_model.lmn_data[..., 0],
        beam_gain_model.lmn_data[..., 1],
        beam_gain_model.lmn_data[..., 2]
    )
    ...
    sc = plt.scatter(geodesics[:, 0, 0, 0], geodesics[:, 0, 0, 1], c=np.log10(np.abs(gain_screen[:, 0, 0, 0, 0, 0])),
                     cmap='PuOr', s=1)
    plt.colorbar(sc)
    plt.xlabel('l')
    plt.ylabel('m')
    plt.title('Reconstructed log10(Amplitude)')
    plt.show()
    sc = plt.scatter(geodesics[:, 0, 0, 0], geodesics[:, 0, 0, 1], c=np.angle(gain_screen[:, 0, 0, 0, 0, 0]),
                     cmap='hsv',
    ...
    np.testing.assert_allclose(
        ...
        np.abs(quantity_to_jnp(beam_gain_model.model_gains[0, :, 0, ...])),
        atol=0.02
    )

    np.testing.assert_allclose(
        np.angle(gain_screen[:, 0, 0, 0, ...]),
        np.angle(quantity_to_jnp(beam_gain_model.model_gains[0, :, 0, ...])),
        atol=0.02
    )
