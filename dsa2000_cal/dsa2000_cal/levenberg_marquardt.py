...
def leaves_vec(tree_x):
    return jnp.concatenate(jax.tree_util.tree_leaves(jax.tree_map(jnp.ravel, tree_x)))
...
class LevenbergMarquardt(base.IterativeSolver):
    ...
    def _cond_fun(self, inputs):
        ...
        if self.stop_criterion == 'madsen-nielsen':
            ...
            rtol_cond = jnp.all(jnp.abs(leaves_vec(state.delta)) <= self.rtol * jnp.abs(leaves_vec(params)))
            grad_cond = jnp.all(jnp.abs(leaves_vec(state.gradient)) <= self.gtol)
    ...
    def init_state(self, init_params: Any, *args,
        ...
        if self.materialize_jac:
            ...
            jtj_diag_max = 1. #self._jtj_diag_max_op(init_params, *args, **kwargs)
            damping_factor = self.damping_parameter * jtj_diag_max
    ...
    def _jtj_diag_max_op_inefficient(self, params, *args, **kwargs):
        """Diagonal elements of J^T.J, where J is jacobian of fun at params."""
        diag_op = lambda v: v.T @ self._jtj_op(params, v, *args, **kwargs)
        return jnp.max(jax.vmap(diag_op)(jnp.eye(len(params))).T)

    def _jtj_diag_max_op(self, params, *args, **kwargs):
        """Efficient computation of diagonal elements of J^T.J without full matrices."""

        # Function to compute the ith diagonal element of J^T.J
        def diag_element(i):
            def f_i(p):
                return self._fun(p, *args, **kwargs)[i]

            grad_fn = jax.grad(f_i)
            gradient_i = grad_fn(params)
            return jnp.dot(gradient_i, gradient_i)

        # Use vmap to efficiently compute all diagonal elements without constructing large matrices
        diag_elements = jax.vmap(diag_element)(jnp.arange(len(params)))

        return jnp.max(diag_elements)
