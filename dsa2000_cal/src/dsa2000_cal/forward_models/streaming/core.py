...
class ForwardModelCore(AbstractForwardModelCore):
    """
    Represents the core of the forward model that runs a single step of the forward model. We formulate as a streaming
    Markov model, where each step consists of a transition from the prior state to the current, followed by a production
    of data. Only the state is persisted between steps, and the data is stored.
    """
