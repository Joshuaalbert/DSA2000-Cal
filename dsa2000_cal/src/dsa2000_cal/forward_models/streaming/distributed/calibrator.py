...
def build_compute_residuals():
    # mesh = create_mesh((D,1,1,1), ('D','T','B','F'),
    #                    devices=)
    #
    # @partial(
    #     shard_map,
...
class Calibrator:
    ...
    async def __call__(self, sol_idx: int) -> CalibratorResponse:
        ...
        async def get_data(time_idxs: List[int], freq_idxs: List[int]):
            data_responses = await asyncio.gather(
                *[
                    self._data_streamer.remote(time_idx, freq_idx)
            ...
            for data_response, (time_idx, freq_idx) in zip(data_responses, zip(time_idxs, freq_idxs)):
                self.vis_data[time_idx, :, freq_idx] = data_response.vis
                self.weights_data[time_idx, :, freq_idx] = data_response.weights
                self.flags_data[time_idx, :, freq_idx] = data_response.flags
                self.freqs[freq_idx] = data_response.freq
                self.times[time_idx] = data_response.time
                self.uvw[time_idx, :, :] = data_response.uvw
        ...
        _, _, previous_solution = await asyncio.gather(
            get_data(time_idxs, freq_idxs),
            get_model_data(time_idxs, freq_idxs),
            self._calibration_solution_cache.get_calibration_solution_snapshot(freq_idxs)
        )
        vis_residuals, solution = self.calibrate_jit(
            vis_data=jnp.asarray(self.vis_data, mp_policy.vis_dtype),
            vis_model=jnp.asarray(self.vis_model, mp_policy.vis_dtype),
            weights_data=jnp.asarray(self.weights_data, mp_policy.weight_dtype),
            flags_data=jnp.asarray(self.flags_data, mp_policy.flag_dtype),
            uvw=jnp.asarray(self.uvw, mp_policy.length_dtype),
        ...
        return CalibratorResponse(
            visibilities=np.asarray(vis_residuals),
