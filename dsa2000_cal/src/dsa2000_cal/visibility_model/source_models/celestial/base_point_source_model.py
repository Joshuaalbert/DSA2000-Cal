...
class PointModelData(NamedTuple):
    ...
    times: jax.Array  # [num_time]
    image: jax.Array  # [num_time, chan, source,[2,2]] in [[xx, xy], [yx, yy]] format or stokes I
...
class BasePointSourceModel(AbstractSourceModel[PointModelData]):
    ...
    def get_model_data(self, freqs: FloatArray, times: FloatArray, geodesic_model: BaseGeodesicModel) -> PointModelData:
        ...
        if self.is_full_stokes():
            image = jnp.where(elevation_mask[None, :, None, None], jnp.zeros_like(image), image)  # [num_freqs, num_sources, 2, 2]
        else:
    ...
    def predict(
        ...
        if gain_model is not None:
            ...
            l_avg = jnp.sum(model_data.lmn[None, None, :, 0, None, None] * model_data.image,
