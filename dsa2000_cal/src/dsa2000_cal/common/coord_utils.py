...
def create_uvw_frame(obs_time: at.Time, phase_center: ac.ICRS, barycentre: str = 'earth') -> ac.SkyOffsetFrame:
    ...
    Args:
        ...
        phase_center: the phase tracking direction
    ...
    frame_uvw = ac.SkyOffsetFrame(origin=phase_center.transform_to(gcrs_frame),
                                  obstime=obs_time,
                                  obsgeoloc=obs_position,
                                  obsgeovel=obs_velocity
                                  )  # GCRS
...
def earth_location_to_uvw_approx(antennas: EarthLocation, obs_time: at.Time, phase_center: ac.ICRS,
    ...
    frame_uvw = create_uvw_frame(
        obs_time=obs_time, phase_center=phase_center,
...
def icrs_to_lmn_old(sources: ac.ICRS, time: at.Time, phase_center: ac.ICRS) -> Quantity:
    """
    ...
    Args:
        ...
        phase_center: the pointing direction
    ...
    phase_center_shape = phase_center.shape
    # Assert that they broadcast
    try:
        np.broadcast_shapes(source_shape, time_shape, phase_center_shape)
    except ValueError:
        raise ValueError(
            f"Shapes of sources {source_shape}, time {time_shape}, "
            f"and phase_center {phase_center_shape} do not broadcast."
        )
...
def icrs_to_lmn(sources: ac.ICRS, phase_center: ac.ICRS) -> Quantity:
    """
    Convert ICRS coordinates to LMN coordinates.
...
def lmn_to_icrs_old(lmn: Quantity, time: at.Time, phase_center: ac.ICRS) -> ac.ICRS:
    ...
    # Assert that they broadcast
    try:
        np.broadcast_shapes(lmn_shape, time_shape, phase_center_shape)
    except ValueError:
        raise ValueError(
...
def lmn_to_icrs(lmn: Quantity, phase_center: ac.ICRS) -> ac.ICRS:
    """
    Convert LMN coordinates to ICRS coordinates.
    ...
    ra, dec = perley_icrs_from_lmn(
        ...
        ra0=quantity_to_jnp(phase_center.ra),
        dec0=quantity_to_jnp(phase_center.dec)
    )
...
def lmn_to_enu(lmn: Quantity, array_location: ac.EarthLocation, time: at.Time, phase_center: ac.ICRS) -> ENU:
    """
    ...
    """
    icrs = lmn_to_icrs(lmn=lmn, phase_center=phase_center)
