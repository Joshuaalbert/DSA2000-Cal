...
def lm_solver(
    ...
    Args:
        residual_fn: the residual function
        x0: an initial guess for the solution
        args: additional arguments to pass to the residual function
        maxiter: maximum number of iterations
        maxiter_cg: maximum number of iterations
        gtol: the stopping criterion
        p_accept: acceptance threshold
        p_lower: lower bound on trust region
        p_upper: upper bound on trust region
        mu_init: initial guess for the solution
        mu_min: smallest allowed value of mu
        approx_grad: whether to reuse J for next gradient evaluation
        verbose: if true then print out during solve
    ...
    def step_fn(state: LMState):
        ...
        (
            x_new, Q_new, R_new, delta_x_prev_new, delta_x_prev2_new
        ) = jax.tree.map(
        ...
        state = LMState(
            ...
            delta_x_prev=delta_x_prev_new, delta_x_prev2=delta_x_prev2_new, iter=state.iter + 1
        )
    ...
    return merge_fn(final_state.x), final_diagnostics
